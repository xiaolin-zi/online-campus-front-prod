  ...
  <div class="login">
    <div class="main">
      ...
      >
        ...
        <div v-if="active==0">
          <p style="margin-top: 30px;color: #79797c">邮箱<span
              style="color: rgb(255, 68, 0)">{{ decryptEmail() }}</span>为确认是你本人操作，请完成以下验证:</p>
          <div style="margin-top: 30px">
            <el-form ref="verifyEmailForm" :model="verifyEmail" label-width="80px">
              <el-form-item
              prop="email"
              :rules="[
                      {
...
export default {
  ...
  setup() {
    ...
    const decryptEmail = () => {
      ...
      const emailNameStart = emailName.substring(0, 2);
      const emailNameEnd = emailName.substring(emailNameLength - 4, emailNameLength);
      return emailNameStart + '****' + emailNameEnd + '@' + emailDomain;
    }


    const verifyEmailForm = ref(null)
    ...
    const getCode = async () => {
      ////验证邮箱是否正确,再判断手机号格式
      verifyEmailForm.value.validateField("email", (valid) => {
        if (!valid) {
          return;
        } else {
